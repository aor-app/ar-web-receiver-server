...
class ARReceiverCore {
    ...
    async setSelectSquelch (level) {
        return await this._executeWrite(`SQ${level}`);
    }    
    async setNoiseSquelch (level) {
        return await this._executeWrite(`NQ${level}`);
    }
    ...
    async getSelectSquelch () {
        let result = await this._executeWrite('SQ');
        return this._getValue(result.resultBuf, 'SQ', 1);
    }
    async getNoiseSquelch () {
        let result = await this._executeWrite('NQ');
        return this._getValue(result.resultBuf, 'NQ', 2);
    }
